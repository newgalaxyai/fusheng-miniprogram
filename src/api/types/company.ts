...
export type ICorp = {
  ...
  socialSecurityStaffNum: string // 参保人数
  regLocation: string // 注册地址
  websites: string // 公司网站
  mainProductTags: string // 主要产品标签
  recommend: boolean // 是否推荐
  // 自定义企业字段
  isJoinClue: boolean // 是否加入线索
  hasFeedback: number // 是否有反馈 0 无反馈 1 有效 2 无效
  // businessScope: string // 业务范围
  // companyScaleInfo: string // 公司规模
  // contactInfo: IContactInfo // 联系方式
  // creditCode: string // 统一社会信用代码
  // establishTime: string // 成立时间
  // gid: number // 企业id
  // industry: string // 行业
  ...
  // regCapital: string // 注册资本
  // regStatus: string // 注册状态
  // staffNum: number // 员工人数
  // tags: string[] // 标签列表
  // websites?: string | null // 公司网站
  // score?: number | string // 匹配度
  // enterpriseAnalysisBack: string // 企业分析背景
  // productMatch: number | null // 产品匹配
  // marketPotential: number | null // 市场潜力
  // riskLevel: number | null // 合作风险
}
// 获取企业列表请求参数
export type IGetCorpListRequest = IPaginationRequest & {
  source: 2 // 小程序
  messageId: number | null // 消息id
  keyword: string
}
